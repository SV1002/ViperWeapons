...
//-----------------------------------------------------------
class X2Action_ViperWeaponsBindEndAlt extends X2Action;
...
var bool									bIsCaster; //this must be implicity set
//*************************************
...
{
	function AnimNodeSequence EndBind(XComGameState_Unit PlayOnGameStateUnit, XGUnit PlayOnUnit, XComUnitPawn PlayOnPawn, bool bUseCasterAnim)
	{
		PlayOnPawn.EnableRMA(true,true);
		PlayOnPawn.EnableRMAInteractPhysics(true);

		Params = default.Params;
		Params.AnimName = ( bUseCasterAnim == true ) ? 'NO_BindCastStop' : 'NO_BindStop';
...
Begin:
	...
	{
		UnitAnimSeq = EndBind(UnitState, Unit, UnitPawn, bIsCaster);
	}

	if( bPartnerIsAliveAndConcious )
	{
		PartnerAnimSeq = EndBind(PartnerUnitState, PartnerVisualizer, PartnerUnitPawn, !bIsCaster);
	}
