...
//-----------------------------------------------------------
class X2Action_ViperWeaponsBind extends X2Action;
...
{
	function AnimNodeSequence PlayBindAnim(XComUnitPawn PlayOnPawn, vector FaceDir, bool bUseCasterAnim)
	{
		...
		Params.AnimName = ( bUseCasterAnim == true ) ? 'NO_BindCastStart' : 'NO_BindStart';
	...
	function PlayBindLoopAnim(XComUnitPawn PlayOnPawn, bool bUseCasterAnim)
	{
		Params = default.Params;
		Params.AnimName = ( bUseCasterAnim == true ) ? 'NO_BindCastLoop' : 'NO_BindLoop';
		Params.Looping = true;
...
Begin:
	...
	ShooterAnim = PlayBindAnim(UnitPawn, (PartnerUnitPawn.Location - UnitPawn.Location), bIsCaster);
	...
	TargetAnim = PlayBindAnim(PartnerUnitPawn, (UnitPawn.Location - PartnerUnitPawn.Location), !bIsCaster);
	...
	PlayBindLoopAnim(UnitPawn, bIsCaster);
	PlayBindLoopAnim(PartnerUnitPawn, !bIsCaster);
